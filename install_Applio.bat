...
set "principal=%cd%"
set "URL_EXTRA=https://huggingface.co/IAHispano/Applio/resolve/e328c93cca1d4454f839e2c6751153cc4e512456/env.zip"
...
if not exist "%cd%\env.zip" (
    ...
    curl -s -LJO %URL_EXTRA% -o env.zip
)

if not exist "%cd%\env.zip" (
    echo Download failed, trying with the powershell method
    powershell -Command "& {Invoke-WebRequest -Uri '%URL_EXTRA%' -OutFile 'mingit.zip'}"
)
